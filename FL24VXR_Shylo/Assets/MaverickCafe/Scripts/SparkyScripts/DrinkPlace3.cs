...
public class DrinkPlace3 : MonoBehaviour
{
    ...
    public GameObject SparkyTalkBox;
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (grabInteractable != null && grabInteractable.isSelected && other.gameObject.name.Contains("TempTea"))
        {
            ...
            Correct.Play();
            Debug.Log($"Collided with Tea: {other.gameObject.name}");
            wrong = false;
        }
        else
        {
            Rigidbody rb = other.GetComponent<Rigidbody>();
            if (rb != null && !other.gameObject.name.Contains("TempTea"))
            {
